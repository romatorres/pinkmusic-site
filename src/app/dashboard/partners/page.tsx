      ...
      <Card className="mb-8">
        ...
        <CardContent>
          <form onSubmit={handleAddPartner} className="space-y-4">
            ...
            <div>
              <Label htmlFor="partnerImage">Logo do Parceiro</Label>
              <Input
                id="partnerImage"
                type="file"
                onChange={(e) => setNewPartnerImage(e.target.files ? e.target.files[0] : null)}
                required
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {partners.map((partner) => (
              <Card key={partner.id} className="flex flex-col items-center p-4">
                <img src={`/partners/${partner.imageUrl}`} alt={partner.name} className="w-24 h-24 object-contain mb-4" />
                <p className="text-lg font-semibold text-center mb-2">{partner.name}</p>
                <Button variant="destructive" onClick={() => handleDeletePartner(partner.id)} className="w-full">
                  Excluir
                </Button>
              </Card>
            ))}
